...
function ResultPage() {
  ...
  useEffect(() => {
    ...
    if (!isNaN(lat) && !isNaN(lng)) {
      ...
      setLocationDescription(
        `이 위치는 위도 ${lat.toFixed(6)}도, 경도 ${lng.toFixed(
          6
        )}도에 위치한 멋진 지역입니다. 이곳은 평소에 잘 알려지지 않은 숨겨진 명소일 수 있습니다.`
      );

      // 활동 추천 설정 (임의의 예시 활동)
      setActivitySuggestions(['근처에서 하이킹', '현지 카페에서 커피 한잔', '주변 명소 탐방', '사진 촬영 명소']);
    }
  ...
  return (
    <div className="page3" style={{ backgroundImage: `url(${process.env.PUBLIC_URL + '/stone-wall.jpg'})` }}>
      {/* 포스터 */}
      <div className="poster">
        ...
        <div className="description-summary description-box">
          ...
          <div className="hidden-details">
            <p>
              이 지역은 현지인들이 자주 찾는 장소입니다. 주변에 다양한 음식점과 휴식 공간이 마련되어 있어 방문객들이
              편하게 쉴 수 있습니다.
            </p>
          </div>
        </div>
        ...
        <div className="description-container">
          ...
          <div className="description-box">
            ...
            <div className="hidden-details">
              <p>
                이 지역은 독특한 건축물과 역사적인 장소로 유명합니다. 현지 문화를 경험할 수 있는 기회도 놓치지 마세요!
              </p>
            </div>
          </div>
        </div>
